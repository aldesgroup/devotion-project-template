...
locals {
  subdomain_name = "devotionprojecttemplate-${var.config.env}.${var.config.domain_name}"
}
...
resource "azurerm_resource_group" "rg" {
  ...
  name     = "rg-???-${var.config.env}-devotionprojecttemplate"
  location = var.config.location
  tags     = { environment = var.config.env, application = "DevotionProjectTemplate" }
}

# 2) Log Analytics
resource "azurerm_log_analytics_workspace" "log" {
  provider            = azurerm.environment_sub
  name                = "log-???-${var.config.env}-devotionprojecttemplate"
  location            = var.config.location
...
resource "azurerm_container_app_environment" "cae" {
  provider                   = azurerm.environment_sub
  name                       = "cae-???-${var.config.env}-devotionprojecttemplate"
  location                   = var.config.location
  resource_group_name        = azurerm_resource_group.rg.name
...
resource "azurerm_user_assigned_identity" "uai" {
  ...
  name                = "id-???-${var.config.env}-devotionprojecttemplate"
  location            = var.config.location
...
resource "azurerm_key_vault" "kv" {
  provider                   = azurerm.identity_sub
  name                       = "kv-???-${var.config.env}-devotionprojecttemplate"
  location                   = var.config.location
...
resource "azurerm_key_vault_certificate" "self_signed" {
  provider     = azurerm.identity_sub
  name         = "kvcert-???-${var.config.env}-devotionprojecttemplate"
  key_vault_id = azurerm_key_vault.kv.id
...
resource "azurerm_container_app_environment_certificate" "cert" {
  provider                     = azurerm.environment_sub
  name                         = "caecert-???-${var.config.env}-devotionprojecttemplate"
...
resource "azurerm_container_app" "aca" {
  provider                     = azurerm.environment_sub
  name                         = "aca-???-${var.config.env}-devotionprojecttemplate"
  resource_group_name          = azurerm_resource_group.rg.name
  container_app_environment_id = azurerm_container_app_environment.cae.id
  ...
  template {
    container {
      name   = "devotion-project-template-api"
      image  = "${var.config.acr_name}.azurecr.io/devotion-project-template-api:latest"
      cpu    = 0.5
      memory = "1Gi"
      args = ["--port", "${var.config.port}"]
      ...
      env {
        name  = "OIDC_AUDIENCE"
        value = "api://${var.config.domain_name}/app-???-devotionprojecttemplate"
      }
    }
  }
